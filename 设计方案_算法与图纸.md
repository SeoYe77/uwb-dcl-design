...
```mermaid
flowchart LR
  subgraph Robot_i[Robot i 节点]
    UWB[UWB测距]:::sensor --> Assoc[邻居识别/数据关联]:::assoc
    VIO[视觉/激光里程计]:::sensor --> Assoc
    Assoc --> IF[信息滤波更新(DKF)]:::fuse
    IF --> CI[CI/SCI一致性融合]:::fuse
    CI --> PGOi[局部位姿图优化]:::backend
    PGOi --> MsgOut[消息打包/发布]:::comm
  end

  subgraph Network[网状网络]
...
| 模块名称 | 颜色图例 | 主要功能 | 输入（量纲/单位） | 输出（量纲/单位） | 备注 |
|---------|----------|---------|-------------------|-------------------|------|
| UWB测距 | 蓝色（传感前端） | 采集邻居间距离 | r_ij（m）；σ_r（m） | 距离约束（m） | 可扩展AoA/TDoA |
| 视觉/激光里程计 | 蓝色（传感前端） | 本机里程计与相对约束来源 | 位姿增量（m, rad）；特征匹配结果 | 里程计先验（m, rad）；候选相对位姿约束（m, rad） | 视觉/激光可任选 |
| 邻居识别/数据关联 | 绿色（数据关联） | 邻居检测、闭环候选筛选 | 传感特征/ID；时戳（s） | 通过的约束集合；质量评分 | 支持JPDA/RANSAC/语义过滤 |
| 信息滤波更新（DKF/IF） | 橙色（分布式融合） | 信息形式更新局部估计 | H、R、约束z（单位随约束） | J（1/单位²）、h（1/单位）；x_i估计（m, rad） | IF便于摘要共享 |
| CI/SCI一致性融合 | ��色（分布式融合） | 避免未知相关造成过度乐观 | 本地(x_i,P_i)与邻居(x_j,P_j) | x_CI（m, rad）；P_CI（m², rad²） | ω∈[0,1]；SCI更不保守 |
| 局部位姿图优化（PGO） | 紫色（分布式后端） | 稳健因子图优化 | 约束z_ij、Σ_ij（m², rad²） | 优化后x_i（m, rad）；残差统计 | 使用Huber/Cauchy损失 |
| 消息打包/发布 | 灰色（通信与网络） | 轻量化摘要与约束广播 | 关键帧、约束、协方差、信息块 | 消息包（字节B） | 降采样/带宽控制 |
| 网状网络接收处理 | 灰色（通信与网络） | 接收与解析邻居消息 | 消息包（B）；时间戳（s） | 约束入库；融合触发 | 异步/丢包容忍 |
